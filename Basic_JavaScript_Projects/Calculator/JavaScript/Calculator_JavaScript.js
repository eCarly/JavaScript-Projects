...
const Calculator = {
    ...
    Display_Value: '0',
    // this will hold the first operand of any expressions, we set it to null for now
    First_Operand: null,
...
function Input_Digit(digit) {
    ...
    if (Wait_Second_Operand === true) {
        ...
        //this overwrites Display_Value if the vurrent value is 0
        //otherwise it adds onto it.
        Calculator.Display_Value = Display_Value === '0' ? digit : Display_Value + digit;
    }
 }
 //this section hadnles decimal points
 function Input_Decimal(dot) {
     //this ensures that accidental clicking of the decimal point 
     // doesn't cause bugs in your operation
     if (Calculator.Wait_Second_Operand === true) return;
     if (!Calculator.Display_Value.includes(dot)) {
         //we are saying that if the Display_Value doe not contain a decimal point
 ...
 function Handle_Operator(Next_Operator) {
     ...
     if (First_Operand == null) {
        ...
        //if operator exists, property lookup is prformed for the operator 
        //in the Perform_Calculation object and the function that matches the
        //operator is executed
        let result = Perform_Calculation[operator](Value_Now, Value_Of_Input);
        //here we add a fixed amount of numbres after the decimal
...
keys.addEventListener('click', (event) => {
    ...
    //ensures that AC clear the numbers from the Calculator
